...
class Worker:
    ...
    # returns a list of lists for each agent $j$. each list represents the agents that will send a message to $j$
    def get_comm_map(self, episode_comm_maps, info):
        this_turns_comm_map = []

        # list all distances
        dists = np.ones([len(info["kilobots"]), len(info["kilobots"])]) * 1000
        for j, agent_j in enumerate(info["kilobots"]):
            for k, agent_k in enumerate(info["kilobots"]):
                if j == k:
                    continue
                dists[j][k] = cartesian_dist(agent_j, agent_k)

        for i in range(self.number_of_agents):
            # choose 3 first agents
            surviving_comms = [0, 1, 2]
            max_dist = max(dists[i][0:3])
            max_index = 0 if dists[i][0] == max_dist else 1 if dists[i][1] == max_dist else 2

            # if another has shorter distance, replace furthest agent away
            for k, distances_of_i in enumerate(dists[i][3:], start=3):
                if distances_of_i < max_dist:
                    surviving_comms[max_index] = k
                if dists[i][surviving_comms[0]] > dists[i][surviving_comms[1]] and \
                                dists[i][surviving_comms[0]] > dists[i][surviving_comms[2]]:
                    max_dist = dists[i][surviving_comms[0]]
                    max_index = 0
                elif dists[i][surviving_comms[1]] > dists[i][surviving_comms[0]] and \
                                dists[i][surviving_comms[1]] > dists[i][surviving_comms[2]]:
                    max_dist = dists[i][surviving_comms[1]]
                    max_index = 1
                else:
                    max_dist = dists[i][surviving_comms[2]]
                    max_index = 2
